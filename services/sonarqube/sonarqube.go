...
func (c *Configurator) Configure(ctx context.Context, opts service.ConfigureOpts) (*service.ConfigureResult, error) {
	if opts.DryRun {
		return &service.ConfigureResult{Status: service.StatusDryRun, Message: "would rotate SonarQube default admin password"}, nil
	}
	if ok, _ := c.validate(ctx, c.AdminPassword); ok {
		return &service.ConfigureResult{Status: service.StatusAlreadyConfigured, Message: "SonarQube admin password already configured"}, nil
	}
	...
	return &service.ConfigureResult{Status: service.StatusUpdated, Message: "SonarQube default admin password rotated"}, nil
}
