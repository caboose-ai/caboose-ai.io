...
func (c *Configurator) Configure(ctx context.Context, opts service.ConfigureOpts) (*service.ConfigureResult, error) {
	if !c.containerExists(ctx) {
		return &service.ConfigureResult{Status: service.StatusSkipped, Message: "Mattermost container not found"}, nil
	}

	if opts.DryRun {
		return &service.ConfigureResult{Status: service.StatusDryRun, Message: "would create Mattermost local admin and default team"}, nil
	}
	...
	if !created && !opts.Force {
		return &service.ConfigureResult{Status: service.StatusAlreadyConfigured, Message: "Mattermost local admin already configured"}, nil
	}
	return &service.ConfigureResult{Status: service.StatusCreated, Message: "Mattermost local admin and team configured"}, nil
}
