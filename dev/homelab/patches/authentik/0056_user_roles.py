...
# Patched: get_role_for_group_id guards against group_id column already being removed
# by authentik_rbac.0010 when upgrading from 2025.x (where 0010 ran before 0056 existed).
# Upstream fix: https://github.com/goauthentik/authentik/pull/21964
...
def migrate_object_permissions(apps: Apps, schema_editor: BaseDatabaseSchemaEditor):
    ...
    def get_role_for_group_id(group_id: int) -> Role:
        # If authentik_rbac.0010 already ran (upgrading from 2025.x), group_id no longer
        # exists in the DB. Fall back to name-only lookup/create.
        try:
            ...
            # group_id column already removed — create/find by name only
            name = f"ak-migrated-role--group-{group_id}"
            role, _ = Role.objects.using(db_alias).get_or_create(name=name)
            return role
