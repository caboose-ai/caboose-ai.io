...
func (c *Configurator) Configure(ctx context.Context, opts services.ConfigureOpts) (*services.ConfigureResult, error) {
	...
	if opts.DryRun {
		return &services.ConfigureResult{Status: services.StatusDryRun, Message: "would write Homarr OIDC credentials to .env"}, nil
	}

	existingID, _ := c.Secrets.Get(ctx, "HOMARR_OIDC_CLIENT_ID")
	if existingID == provider.ClientID && !opts.Force {
		return &services.ConfigureResult{Status: services.StatusAlreadyConfigured, Message: "Homarr OIDC already configured"}, nil
	}
