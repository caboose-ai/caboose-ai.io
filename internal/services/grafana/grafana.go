...
func (c *Configurator) Configure(ctx context.Context, opts services.ConfigureOpts) (*services.ConfigureResult, error) {
	...
	if opts.DryRun {
		return &services.ConfigureResult{Status: services.StatusDryRun, Message: "would write Grafana OAuth credentials to .env"}, nil
	}

	existingID, _ := c.Secrets.Get(ctx, "GF_AUTH_GENERIC_OAUTH_CLIENT_ID")
	if existingID == provider.ClientID && !opts.Force {
		return &services.ConfigureResult{Status: services.StatusAlreadyConfigured, Message: "Grafana OAuth already configured"}, nil
	}
