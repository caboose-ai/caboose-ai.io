...
func (c *Configurator) Configure(ctx context.Context, opts services.ConfigureOpts) (*services.ConfigureResult, error) {
	...
	if opts.DryRun {
		return &services.ConfigureResult{Status: services.StatusDryRun, Message: "would write Open WebUI OIDC credentials to .env"}, nil
	}

	existingID, _ := c.Secrets.Get(ctx, "OPEN_WEBUI_OAUTH_CLIENT_ID")
	if existingID == provider.ClientID && !opts.Force {
		return &services.ConfigureResult{Status: services.StatusAlreadyConfigured, Message: "Open WebUI OIDC already configured"}, nil
	}
