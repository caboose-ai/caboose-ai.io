...
func (c *Configurator) Configure(ctx context.Context, opts services.ConfigureOpts) (*services.ConfigureResult, error) {
	...
	if !setup {
		return &services.ConfigureResult{Status: services.StatusAlreadyConfigured, Message: "n8n owner account already configured"}, nil
	}
	if opts.DryRun {
		return &services.ConfigureResult{Status: services.StatusDryRun, Message: "would create n8n owner account"}, nil
	}
	...
	return &services.ConfigureResult{Status: services.StatusCreated, Message: "n8n owner account created"}, nil
}
