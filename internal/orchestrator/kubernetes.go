...
func (b *KubernetesBackend) kubectlArgs(args ...string) []string {
	prefix := []string{}
	if b.cfg.Kubernetes.Kubeconfig != "" {
		prefix = append(prefix, "--kubeconfig", b.cfg.Kubernetes.Kubeconfig)
	}
	if b.cfg.Kubernetes.Context != "" {
		prefix = append(prefix, "--context", b.cfg.Kubernetes.Context)
	}
...
func (b *KubernetesBackend) Apply(ctx context.Context) error {
	if err := b.renderManifest(); err != nil {
		return err
	}
	if _, err := b.runner.Run(ctx, "kubectl", b.kubectlArgs("create", "namespace", b.cfg.Kubernetes.Namespace, "--dry-run=client", "-o", "yaml")...); err != nil {
		return err
	}
	if _, err := b.runner.Run(ctx, "kubectl", b.kubectlArgs("apply", "-f", "-")...); err != nil {
		return err
	}
	if _, err := b.runner.Run(ctx, "kubectl", b.kubectlArgs("apply", "-f", b.manifestDir)...); err != nil {
		return err
	}
	return nil
}
