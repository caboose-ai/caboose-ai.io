...
func RunInstall(ctx context.Context, inst *install.Installer) int {
	...
	if dryRun {
		...
		console.Run("would leave Paperclip example stopped until requested with `mise run paperclip:example`")
	...
	console.Info("Paperclip example remains stopped", "run", "mise run paperclip:example")
