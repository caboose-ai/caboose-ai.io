package smoketest

import "testing"

func TestIsLoggedInURL(t *testing.T) {
	host := "openclaw.example.com"

	for _, tc := range []struct {
		name string
		url  string
		want bool
	}{
		{name: "landing", url: "https://openclaw.example.com/chat?session=main", want: true},
		{name: "authentik user library", url: "https://auth.example.com/if/user/#/library", want: true},
		{name: "login", url: "https://openclaw.example.com/login", want: false},
		{name: "auth", url: "https://openclaw.example.com/auth", want: false},
		{name: "error", url: "https://openclaw.example.com/error", want: false},
		{name: "link account", url: "https://openclaw.example.com/user/link_account", want: false},
		{name: "auth hash", url: "https://openclaw.example.com/#!/auth", want: false},
		{name: "wrong host", url: "https://idp.example.com/", want: false},
		{name: "redirect query on wrong host", url: "https://idp.example.com/flow/default/?next=https%3A%2F%2Fopenclaw.example.com%2Fchat", want: false},
	} {
		t.Run(tc.name, func(t *testing.T) {
			targetHost := host
			if tc.name == "authentik user library" {
				targetHost = "auth.example.com"
			}
			if got := isLoggedInURL(tc.url, targetHost); got != tc.want {
				t.Fatalf("isLoggedInURL(%q, %q) = %v, want %v", tc.url, targetHost, got, tc.want)
			}
		})
	}
}
